...
resource "aws_codebuild_project" "containerAppBuild" {
  badge_enabled  = false
  build_timeout  = 60
  name           = var.nodejs_project_repository_name
  queued_timeout = 480
...
resource "aws_codecommit_repository" "test" {
  ...
  default_branch  = var.nodejs_project_repository_branch
}
...
resource "aws_codepipeline" "node_app_pipeline" {
  ...
  depends_on = [aws_ecs_cluster.main,aws_codecommit_repository.test]
}
